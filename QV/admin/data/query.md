...
以下按原题顺序补全，共 68 题。每题回答以约 30–60 秒口述为目标；技术名称较多时适当放慢，不必逐字背诵。先说核心判断，再用项目或例子展开，追问时再补实现细节。

项目口径依据当前简历、`liu.md` 和导诊方案：导诊项目重点讲文档解析、信息抽取、LangGraph 编排及记忆架构；RAG 对应颈椎影像项目中的 BGE、Reranker、Elasticsearch 和报告生成。方案与原型的完成程度存在差异，因此未确认的流式、Trace 平台、持久化、混合召回和动态模型路由等能力按设计思路回答，不作为已上线成果。实际做过的部分，可用真实实现和测试记录替换相应表述。

“备考备注”用于提示事实边界或查阅资料，不属于面试口述内容。保留第一部分原有跳号，便于对应题单；第二部分修正明显编号笔误及截断题目。
...
> 备考备注（不口述）：本题是约一分钟短版；3–4 分钟完整版见 [liu.md](liu.md)。
